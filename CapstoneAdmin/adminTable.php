...
<head>
  ...
  <!-- Favicons -->
  <link href="/CapstoneAdmin/assets/img/logomini.png" rel="icon">
      ...
      <section id="add" class="hero d-flex align-items-center  " style= "padding-top: 0;">
       <div class="container">
              <div class="row">
                  ...
                  <h2><span>Available Dog</span>  Records </h2> 
                  <div class="row mt-3">
                ...
                <?php
                  ...
                  while ($row = $result->fetch_assoc()) {
                      echo "<tr>
                          <td>{$row['id']}</td>
                          <td><img src='data:image/jpeg;base64," . base64_encode($row['dog_image']) . "' width='50' height='50' alt='Dog Image'></td>
                          <td>{$row['name']}</td>
                          <td>{$row['age']}</td>
                          <td>{$row['color']}</td>
                          <td>{$row['gender']}</td>
                          <td>{$row['date_obtained']}</td>
                          <td>{$row['background']}</td>
                          <td>
                          
               
                          <div class='btn-group float-end'>
                          <a class='btn btn-outline-primary' title='Livestream Pet' href='live.php?id=".$row['id']."' ><i class='fas fa-video'></i></a>
                          
                              <a class='btn btn-outline-success' href='#' data-bs-toggle='modal' data-bs-target='#editModal'
                              data-id='{$row['id']}'
                              data-name='{$row['name']}'
                              data-age='{$row['age']}'
                              data-color='{$row['color']}'
                              data-gender='{$row['gender']}'
                              data-date_obtained='{$row['date_obtained']}'
                              data-background='{$row['background']}'
                              >
                              <i class='bi bi-pencil-square'></i> 
                              </a>
                              <a class='btn btn-outline-danger delete-button' href='#' data-bs-toggle='modal' data-bs-target='#deleteModal'
                              data-id='{$row['id']}'
                              data-name='{$row['name']}'
                              data-age='{$row['age']}'
                              data-color='{$row['color']}'
