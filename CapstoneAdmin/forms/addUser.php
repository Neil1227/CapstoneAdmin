...
if (isset($_POST["btn_reg"])) {
    ...
    if (strlen($pass_word) < 8) {
        $adduserSuccess = "Password must be at least 8 characters long.";
    } else {
        // Check if the password and re-entered password match
        if ($pass_word !== $pass_word_reentered) {
            $adduserSuccess = "Passwords do not match. Please re-enter your password.";
        } else {
            // Hash the password
            ...
            if ($result_check_username->num_rows > 0) {
                $adduserSuccess = "Username already exists. Please choose a different username.";
            } else {
                // Insert a new record using a prepared statement
                $sql_insert_user = "INSERT INTO user_tbl (first_name, middle_name, last_name, age, sex, address, username, password, owns_dog, security_question, security_answer) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                if ($stmt_insert_user->execute()) {
                    $adduserSuccess = "User registration successful!";
                } else {
                    $adduserSuccess = "Error updating data: " . $stmt_insert_user->error;
                }
            }
